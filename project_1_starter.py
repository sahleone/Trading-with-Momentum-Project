...
def compute_log_returns(prices):
    ...
    return prices.apply(np.log).diff(1)
...
def get_top_n(prev_returns, top_n = 3):
    """
    ...
    min_val =[row.nlargest(top_n).min() for idx, row in prev_returns.iterrows()]

    return (prev_returns >= min_val).astype(int)
...
def analyze_alpha(expected_portfolio_returns_by_date):
    ...
    t_value,p_value = stats.ttest_1samp(expected_portfolio_returns_by_date, 0.0)
    p_value = p_value/2
...
# As the p-value is greater than the significance level alpha = 0.05, we do not reject the null hypothesis.  
